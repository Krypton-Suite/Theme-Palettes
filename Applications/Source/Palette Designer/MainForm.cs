...
public partial class MainForm : KryptonForm
{
    ...
    private string? _collectionThemeName;
    ...
    private Image _paletteDropperImage;
    private readonly string? _startupPalettePath;
    ...
    {
        InitializeComponent();
        KryptonPaletteFile.EnsureShellAssociations(TryGetOpenWithExecutable());
        InsertEditPackMenuItem();
        _startupPalettePath = GetCommandLinePalettePath();
        ...
        // Initialize palette application lists. The full control lists are
        // configured elsewhere; initialize to empty lists to ensure the
        // constructor compiles and the controls can be populated later.
        _applyPalettesToBases = new List<VisualControlBase>();
        _applyPalettesToPanels = new List<KryptonPanel>();
    ...
    private static string? TryGetOpenWithExecutable()
    {
        var path = Application.ExecutablePath;
        if (IsStandaloneWinExe(path))
        {
            return path;
        }
        ...
        return IsStandaloneWinExe(path) ? path : null;
    }

    private static bool IsStandaloneWinExe(string? path) =>
        !string.IsNullOrWhiteSpace(path)
        && File.Exists(path)
        && string.Equals(Path.GetExtension(path), @".exe", StringComparison.OrdinalIgnoreCase)
        && !string.Equals(Path.GetFileName(path), @"dotnet.exe", StringComparison.OrdinalIgnoreCase);

    private static string? GetCommandLinePalettePath()
    {
        ...
        for (var i = 1; i < args.Length; i++)
        {
            ...
            if (KryptonPaletteFile.IsPaletteExtension(candidate)
                || string.Equals(Path.GetExtension(candidate), @"." + KryptonPaletteFile.XmlExtension,
    ...
    private void Open()
    {
        ...
        try
        {
            TryOpenPaletteFile(dialog.FileName, addToRecentList: true);
        }
        finally
        {
    ...
    private void Save()
    {
        ...
        if (_loaded)
        {
            ...
            {
                KryptonMessageBox.Show(this, ex.Message, @"Palette Save",
    ...
    private void SaveAs()
    {
        ...
        if (_loaded && File.Exists(_filename))
        {
        ...
        if (File.Exists(filename) && KryptonPaletteFile.IsCollection(filename))
        {
            var confirm = KryptonMessageBox.Show(this,
                $@"'{Path.GetFileName(filename)}' is a multi-theme .ktheme collection. Saving will replace the entire pack with this single theme."
                + "\n\nUse File > Edit .ktheme collection... to add or update a named theme in the collection.",
                @"Replace palette collection?",
    ...
    private void rebuildPaletteMappingToolStripMenuItem_Click(object? sender, EventArgs e)
    {
        if (_palette == null)
        {
            KryptonMessageBox.Show(this, @"No palette is currently loaded.", @"Rebuild Palette Mapping",
        ...
        try
        {
            ...
            KryptonMessageBox.Show(this, $"Mapping regenerated ({count} entries).", @"Rebuild Palette Mapping",
                KryptonMessageBoxButtons.OK, KryptonMessageBoxIcon.Information);
        }
        catch (Exception ex)
        {
            KryptonMessageBox.Show(this, "Failed to regenerate mapping:\r\n" + ex.Message,
                @"Rebuild Palette Mapping", KryptonMessageBoxButtons.OK, KryptonMessageBoxIcon.Error);
        }
        finally
        {
            Cursor = previous;
    ...
    private bool TryOpenPaletteFile(string path, bool addToRecentList, string? preferredThemeName = null)
    {
        try
        {
            var palette = new KryptonCustomPaletteBase();
            string? collectionTheme = null;
            ...
            if (KryptonPaletteFile.IsCollection(path))
            {
                ...
                var themeName = names.FirstOrDefault(name =>
                    string.Equals(name, preferredThemeName, StringComparison.OrdinalIgnoreCase));
                if (string.IsNullOrEmpty(themeName))
                {
                    themeName = names.Length == 1
                        ? names[0]
                        : PromptCollectionTheme(path, names);
                }
                ...
                palette.Import(path, selectedTheme, silent: true);
                collectionTheme = selectedTheme;
            }
            else if (_settingsManager.GetUpgradeOnImport())
            {
                var resolved = KryptonPaletteFile.PromptLegacyXmlUpgrade(path, silent: false);
                if (resolved == null)
                {
                    return false;
                }
            ...
            else
            {
                ...
                palette.Import(resolved, silent: true);
            }

            _collectionThemeName = collectionTheme;
            InitializeLoadedPalette(palette, path, addToRecentList);
            return true;
        }
        catch (Exception ex)
        {
            KryptonMessageBox.Show(this, ex.Message, @"Palette Open",
                KryptonMessageBoxButtons.OK, KryptonMessageBoxIcon.Error);
            return false;
        }
    }
    ...
    private void SaveToCurrentPath()
    {
        ...
        if (File.Exists(_filename) && KryptonPaletteFile.IsCollection(_filename))
        {
            var themeName = _collectionThemeName ?? _palette.GetPaletteName();
            if (string.IsNullOrWhiteSpace(themeName))
            {
                themeName = Path.GetFileNameWithoutExtension(_filename);
            }
    ...
    private void InsertEditPackMenuItem()
    {
        var item = new ToolStripMenuItem(@"Edit .ktheme collection...")
        {
            Name = @"menuEditKpalCollection"
        };
        item.Click += MenuEditKpalPack_Click;
        var index = menuFile.DropDownItems.IndexOf(menuSaveAs);
        menuFile.DropDownItems.Insert(index < 0 ? menuFile.DropDownItems.Count : index + 1, item);
    }

    private void MenuEditKpalPack_Click(object? sender, EventArgs e)
    {
        string? path = null;
        if (_loaded
            && File.Exists(_filename)
            && string.Equals(Path.GetExtension(_filename), "." + KryptonPaletteFile.BinaryExtension,
                StringComparison.OrdinalIgnoreCase))
        {
            path = _filename;
        }
        ...
        if (path == null || !File.Exists(path) || string.IsNullOrEmpty(_collectionThemeName) || _dirty)
        {
            return;
        }
        ...
        var names = KryptonPaletteFile.GetThemeNames(path);
        if (!names.Any(name => string.Equals(name, _collectionThemeName, StringComparison.OrdinalIgnoreCase)))
        {
            KryptonMessageBox.Show(this,
                $"Theme '{_collectionThemeName}' is no longer in the collection.",
                @"Edit .ktheme collection",
        ...
        TryOpenPaletteFile(path, addToRecentList: false, preferredThemeName: _collectionThemeName);
    }
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(_startupPalettePath))
        {
            TryOpenPaletteFile(_startupPalettePath!, addToRecentList: true);
        }
    ...
    {
        var title = string.IsNullOrEmpty(_collectionThemeName)
            ? _filename
            : $"{_filename} [{_collectionThemeName}]";
        Text = $@"Palette Designer - {title}{(_dirty ? "*" : string.Empty)}";
    }
    ...
    private void CopyColorsFromBasePalette()
    {
        ...
        try
        {
            ...
            try
            {
                _palette.ButtonStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.CalendarDay.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.ButtonSpecs.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.ControlStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.ContextMenu.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.DragDrop.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.FormStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.GridStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.HeaderStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            } // This one throws
            ...
            try
            {
                _palette.HeaderGroup.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.Images.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.InputControlStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.LabelStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.Navigator.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.PanelStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.Ribbon.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.SeparatorStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.TabStyles.PopulateFromBase(_palette.Common);
            }
            catch
            {
            }
            ...
            try
            {
                _palette.TrackBar.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.ToolMenuStatus.PopulateFromBase();
            }
            catch
            {
            }
            ...
            try
            {
                _palette.CueHintText.PopulateFromBase(PaletteState.Normal);
            }
            catch
            {
            }
    ...
    private void MyOwnRecentPaletteFileGotClicked_Handler(object sender, EventArgs e)
    {
        ...
        TryOpenPaletteFile(fileName!, addToRecentList: false);
    }
            ...
            if (selItem?.PropertyDescriptor != null && selItem.PropertyDescriptor.PropertyType == typeof(Color))
            {
                ...
                if (keyData == (Keys.Control | Keys.V))
                {
                    ...
                    if (TryParseColorString(clip, out var newCol))
                    {
                        try
                        {
                            selItem.PropertyDescriptor.SetValue(
                                (selItem.Parent?.Value ?? propertyGrid.SelectedObject)!, newCol);
                            propertyGrid.Refresh();
                            ApplyPalette();
                        }
                        catch
    ...
    private void SetupContextMenu()
    {
        ...
        _autoFillMenuItem = new ToolStripMenuItem("Auto-Fill Color in Grid")
    ...
    {
        _paletteDropperImage = KryptonScreenColorPicker.CreateDropperGlyphImage();
    }

    private void randomizeToolStripMenuItem_Click(object sender, EventArgs e)
    {
        var randomSeed = KryptonCustomThemeGenerator.CreateRandomSeed();

        if (_palette != null)
        {
            // Create a fresh random custom palette and initialize it as the
            // currently loaded palette so events, property grid and mappings
            // are correctly wired. This replaces the previous direct call to
            // ApplyScheme which expected a different type.
            var randomPalette = KryptonCustomThemeGenerator.Create(randomSeed);
            _collectionThemeName = null;
            InitializeLoadedPalette(randomPalette, "(Randomized)", addToRecentList: false);
        }
    }
}